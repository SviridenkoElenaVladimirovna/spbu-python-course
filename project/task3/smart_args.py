...
def smart_args(func: Callable) -> Callable:
    ...
    for name, param in sig.parameters.items():
        ...
        if isinstance(default, Evaluated) and isinstance(default, Isolated):
            raise AssertionError(
                f"Argument '{name}' cannot use both Evaluated and Isolated"
            )
    ...
    def wrapper(*args, **kwargs):
        ...
        if args:
            raise ValueError(
                "Function with @smart_args must be called with keyword arguments only"
            )

        for name in isolated_params:
            if name not in kwargs:
                raise ValueError(
                    f"Argument '{name}' with Isolated() must be explicitly provided"
                )
