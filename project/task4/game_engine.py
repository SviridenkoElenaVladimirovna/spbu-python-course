...
class GameEngine:
    ...
    def __init__(self, players: List[Player], target_score: int = 5000, max_rounds: int = 50) -> None:
        """
    ...
    def play_round(self, player: Player) -> bool:
        ...
        while not round_ended and dice_count > 0:
            ...
            print(f"points per move: {round_points}, Total in the round: {player.round_score}")
            ...
            if dice_count > 0:
                continue_playing = player.decide_continue(player.round_score, dice_count)
